...
if(!function_exists('gt3_get_pf_type_output')) {
	function gt3_get_pf_type_output($pf, $width, $height, $featured_image, $cols = false){
		...
		if(function_exists('gt3_get_image_srcset') && (bool) $cols) {
			...
			$gt3_featured_image_url_srcset = 'src="'.esc_url(aq_resize($featured_image[0], $width, $height, true, true, false)).'"';
		}

		$featured_standard = '<div class="blog_post_media">'.(!(is_single() && get_post_type() == 'post') ? '<a href="'.esc_url(get_permalink()).'">' : '').'<img '.$gt3_featured_image_url_srcset.' alt="'.esc_attr($alt_text).'" />'.(!(is_single() && get_post_type() == 'post') ? '</a>' : '').'</div>';
		...
		if($page_title_conditional == 'yes' && !empty($gt3_page_title)) {
			...
			*/
			...
			if(is_single() && get_post_type() == 'portfolio') {
				...
				if (true === gt3_option("page_title_meta_conditional")) {
					echo "<div class='page_title_meta cpt_portf gt3_post_title_mata_block'>".$item_category."</div>";
				}
			}
