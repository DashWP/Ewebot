...
// Infinite Scrolling Gradient Text Color
add_action('elementor/element/gt3-core-infinite-scrolling-text/section_text_style_section/before_section_end', function($element,$args){
	$element->add_control(
		'enable_theme_textgradient_scroll',
		array(
			'label'       => esc_html__('Enable Text Color Gradient?', 'ewebot'),
			'type'        => Elementor\Controls_Manager::SWITCHER,
			'description' => esc_html__('If checked, enable text color gradient', 'ewebot'),
	...
	$element->add_control(
		...
		array(
			...
			'default' => '#ff4c6c',
			'selectors'   => array(
				'{{WRAPPER}} .gt3-scrolling-text-element' => '--textgradient_color1: {{VALUE}};',
			),
			'condition' => array(
				'enable_theme_textgradient_scroll!' => '',
			),
	...
	$element->add_control(
		...
		array(
			...
			'default' => '#fa9d4d',
			'selectors'   => array(
				'{{WRAPPER}} .gt3-scrolling-text-element' => '--textgradient_color2: {{VALUE}};',
			),
			'condition' => array(
				'enable_theme_textgradient_scroll!' => '',
			),
	...
	$element->update_control('text_color',array(
		'condition' => array(
			'enable_theme_textgradient_scroll' => '',
		),
	));
...
add_action('elementor/element/gt3-core-TestimonialsLite/general_section/before_section_end', function($element,$args){
	$element->add_control(
		'testimonials_quote',
		array(
			'label' => esc_html__('Disable Quote Icon', 'soulheal'),
			'type'  => Elementor\Controls_Manager::SWITCHER,
			'prefix_class' => 'gt3-quote-disable-',
			'default'     => '',
		),
		array(
			'position' => array(
...
// Blogboxed
add_action('elementor/element/gt3-core-blogboxed/style_section/before_section_end', function($element,$section_id){
	$element->add_control(
		'block_rounded',
		array(
			'label' => esc_html__('Disable Items Rounded', 'soulheal'),
			'type'  => Elementor\Controls_Manager::SWITCHER,
			'prefix_class' => 'gt3-rounded-disable-',
			'default'     => '',
			'condition' => array(
				'module_type' => 'type1',
			),
		...
		[
			...
			'fields_options' => array(
				...
				'gradient_angle' => [
					...
					'selectors' => [
						'{{WRAPPER}}.elementor-widget-gt3-core-blogboxed .module_type1 .item_wrapper' => 'background-color: transparent; background-image: linear-gradient({{SIZE}}{{UNIT}}, {{color.VALUE}} {{color_stop.SIZE}}{{color_stop.UNIT}}, {{color_b.VALUE}} {{color_b_stop.SIZE}}{{color_b_stop.UNIT}});',
					],
				],
			),
			'default' => '',
